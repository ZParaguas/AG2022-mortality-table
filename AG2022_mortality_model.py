...
ANIMAL_SPECIES = ['Olifant', 'Mens', 'Slang', 'Hippo']

def import_experience_mortality(animal_species):

    """
    Desc:
        Import experience mortality for chosen species
    """
    
    iSpecies_index = ANIMAL_SPECIES.index(animal_species)
    dfParamsES_temp = pd.read_excel(r"data\Experience_mortality.xlsx", sheet_name = "Males", index_col=0, engine='openpyxl')
    dfParamsES = dfParamsES_temp.iloc[:,iSpecies_index].values.reshape(1,121).T
...
def period_life_table(mMortalityTable, dfExperimentalCorrection):
    
    """
    Desc:
        Generate derived period life exectancies from AG2022 table and correct for subpopulation with experimetal mortality vector
    """
    
    # Cumulative product at age x for year t
    mCorrectedMortality = dfExperimentalCorrection * (1-mMortalityTable)
    mPeriodLifeExpectancy = np.flipud(np.flipud(np.cumprod(mCorrectedMortality, axis=0)).cumsum(axis=0)) + 0.5

    return mPeriodLifeExpectancy
...
def generate_tables(number_sims, dfParams, dfParamsCov, dfExperimentalCorrection, iStochastic):
    ...
    for _ in range(number_sims):
        ...
        mSimulated_table = period_life_table(mMortality_table, dfExperimentalCorrection)
        
        vLife_tables.append(mSimulated_table)
    
    return vLife_tables

def generate_prognosis_data(number_sims, animal_species):
    ...
    dfExperimentalCorrection = import_experience_mortality(animal_species)    
    dfParamsM, dfParamsV, dfParamsCov = import_systems()
    
    mBestEstimate_temp = generate_tables(1, dfParamsM, dfParamsCov, dfExperimentalCorrection, iStochastic = 0)
    mSimulatedTables_temp = generate_tables(number_sims, dfParamsM, dfParamsCov, dfExperimentalCorrection, iStochastic = 1)
